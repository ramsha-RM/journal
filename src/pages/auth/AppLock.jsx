...
import { changeLockPreferences } from "../../service/lockApi.service";
...
const AppLock = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      localStorage.setItem("app_lock", apiVal);
      
      setMessage({ type: "success", text: "Auto-lock duration updated!" });
      navigationTimeoutRef.current = setTimeout(() => navigate("/dashboard"), 1500);
    } catch (error) {
      const errorMsg = error?.response?.data?.[0] || error?.message || "Failed to update";
